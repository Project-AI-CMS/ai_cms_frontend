...
export function AssetDetail({ assetId, onBack, onEdit }: AssetDetailProps) {
  ...
  const fetchAssetDetails = async () => {
    ...
    try {
      ...
      // Fetch related data using schema camelCase properties
      const allAssetsResp = await assetApi.getAll({});
      const allAssetsList = Array.isArray(allAssetsResp)
        ? allAssetsResp
        : allAssetsResp.data || [];

      const [typeData, locationData] = await Promise.all([
        assetTypeApi.getById(assetData.assetTypeId),
        locationApi.getById(assetData.locationId),
      ]);

      setAssetType(typeData);
      setLocation(locationData);

      // Find parent and children (use camelCase keys)
      if (assetData.parentAssetId) {
        const parent = await assetApi.getById(assetData.parentAssetId);
        setParentAsset(parent);
      }

      const children = (allAssetsList as Asset[]).filter(
        (a: Asset) => a.parentAssetId === assetId
      );
  ...
  // No health/status fields on Asset schema; removed related helpers
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="details" className="w-full">
        ...
        <TabsContent value="details" className="space-y-4">
          ...
          {/* Status information removed (not part of Asset schema) */}
        </TabsContent>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-3 p-4 border-2 border-blue-500 rounded-lg bg-blue-50">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500">{asset.serialNumber}</p>
                </div>
              </div>
            </div>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              {[
                {
                  date: "2025-10-15",
                  event: "Status changed to Operational",
                  user: "Zhang Wei",
                },
                {
                  date: "2025-10-10",
