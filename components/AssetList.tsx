          ...
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-3 px-4 text-sm text-slate-600">Model Number</th>
                    <th className="text-left py-3 px-4 text-sm text-slate-600">Serial Number</th>
                    <th className="text-left py-3 px-4 text-sm text-slate-600">Health Score</th>
                ...
                <tbody>
                  {assets.map((asset) => (
                    <tr key={asset.id} className="border-b hover:bg-gray-50">
                      <td className="py-3 px-4 text-sm text-slate-900">{asset.id}</td>
                      <td className="py-3 px-4">
                        <div>
                          <p className="text-sm text-slate-900">{asset.name}</p>
                          {asset.manufacturer && (
                            <p className="text-xs text-slate-500">{asset.manufacturer}</p>
                          )}
                        </div>
                      </td>
                      <td className="py-3 px-4 text-sm text-slate-900">{asset.model_number}</td>
                      <td className="py-3 px-4 text-sm text-slate-900">{asset.serial_number}</td>
                      <td className="py-3 px-4">
                        <span className={`text-sm font-medium ${getHealthScoreColor(asset.current_health_score)}`}>
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t">
              <p className="text-sm text-slate-600">
                Showing {((currentPage - 1) * limit) + 1} to {Math.min(currentPage * limit, total)} of {total} assets
              </p>
